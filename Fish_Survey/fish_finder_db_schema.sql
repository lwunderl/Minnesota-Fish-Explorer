--Drop Tables
DROP TABLE if EXISTS water_access_sites, median_cpue, fish_catch, fish_length, lake_id, fish_id;

--Create Fish ID table
CREATE TABLE fish_id (
    fish_description VARCHAR(30),
    fish_id PRIMARY KEY
);

--Create Lake ID table
CREATE TABLE lake_id (
...
CREATE TABLE fish_length (
    ...
    FOREIGN KEY (lake_id) REFERENCES lake_id(lake_id),
    FOREIGN KEY (species) REFERENCES fish_id(fish_id)
);
